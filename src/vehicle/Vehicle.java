 ...
 */
public abstract class Vehicle implements Task, Drawable, Comparable {
	 ...
	 * @param percentage
	 *            The percentage of the road it has currently completed
	 * @param m
	 *            The model to use
	 * @param a
	 *            The algorithm that the car should use for path finding
	 */
	...
	public void draw(Graphics g) {
		...
		g.drawOval(
				(int) ((start.getX() * GraphicsSetting.getInstance().getZoom() + differentX * percentage - width / 2)
						* GraphicsSetting.getInstance().getZoom()) + GraphicsSetting.getInstance().getPanX(),
				(int) ((start.getY() * GraphicsSetting.getInstance().getZoom() + differentY * percentage - height / 2)
						* GraphicsSetting.getInstance().getZoom()) + GraphicsSetting.getInstance().getPanY(),
				width, height);
	}
	...
	/**
	 * The index of the current car. Also should be used as the key in
	 * vehicleHolder
	 */
	private int index;
