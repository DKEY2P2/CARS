...
import algorithms.trafficlights.ann.GreedyMLP;
...
/**
 * Adds one property to the smart traffic light which is that it now measures
 * how long it has been green for
 */
public class SmartTrafficLightExtended extends SmartTrafficLight {
    ...
    public void incrementCurrentGreenTime(double currentGreenTime) {
        this.currentGreenTime += currentGreenTime;
        ((GreedyMLP.SimulatedPerfectDector) getDetector()).setWhatToOutput(
                ((GreedyMLP.SimulatedPerfectDector) getDetector())
                        .getWhatToOutput() - 1);
    }
